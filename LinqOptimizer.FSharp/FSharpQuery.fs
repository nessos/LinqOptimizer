    ...
    type Query =
        ...
        static member map<'T,'R> (projection : Expression<Func<'T,'R>>) = 
            fun (query : IQueryExpr<seq<'T>>) -> 
                QueryExpr<seq<'R>>(Transform(projection, query.Expr)) :> IQueryExpr<seq<'R>> 

        static member mapi<'T, 'R>(selector : Expression<Func<'T, int, 'R>>) =
            fun (query : IQueryExpr<seq<'T>>) -> 
                QueryExpr<seq<'R>>(TransformIndexed(selector, query.Expr)) :> IQueryExpr<seq<'R>> 
        ...
        static member fold(func : Expression<Func<'Acc, 'T, 'Acc>>) =
            fun (seed : 'Acc) (query : IQueryExpr<seq<'T>>) ->
                QueryExpr<'Acc>(Aggregate(Expression.Constant(seed), func, query.Expr)) :> IQueryExpr<'Acc> 
        ...
        static member sortBy<'T, 'Key>(keySelector : Expression<Func<'T, 'Key>>) = 
            fun (queryExpr : IQueryExpr<seq<'T>>) ->
                new QueryExpr<seq<'T>>(QueryExpr.OrderBy([keySelector :> LambdaExpression, Order.Ascending], queryExpr.Expr)) :> IQueryExpr<_>
