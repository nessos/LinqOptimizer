    ...
    module internal CSharpExpressionOptimizer =
        ...
        let rec toQueryExpr (expr : Expression) : QueryExpr =
            ...
            | MethodCall (_, MethodName "Select" _, [expr'; Lambda ([_], bodyExpr) as f']) -> 
                Transform (optimize f' :?> LambdaExpression, (toQueryExpr << optimize) expr', bodyExpr.Type)
    
            | MethodCall (_, MethodName "Select" _, [expr'; Lambda ([_; _], bodyExpr) as f']) -> 
                TransformIndexed (optimize f' :?> LambdaExpression, (toQueryExpr << optimize) expr', bodyExpr.Type)
    
            | MethodCall (_, MethodName "Where" _, [expr'; Lambda ([paramExpr], _) as f']) -> 
                Filter (optimize f' :?> LambdaExpression, (toQueryExpr << optimize) expr', paramExpr.Type)
    
            | MethodCall (_, MethodName "Where" _, [expr'; Lambda ([paramExpr; indexExpr], _) as f']) -> 
                FilterIndexed (optimize f' :?> LambdaExpression, (toQueryExpr << optimize) expr', paramExpr.Type)
    
            | MethodCall (_, MethodName "Take" _, [expr'; countExpr]) when countExpr.Type = typeof<int> -> 
                let queryExpr = (toQueryExpr << optimize) expr'
            ...
            | MethodCall (_, (MethodName "SelectMany" [|_; _|] as m), [expr'; Lambda ([paramExpr], bodyExpr)]) -> 
                NestedQuery ((paramExpr, (toQueryExpr << optimize) bodyExpr), (toQueryExpr << optimize) expr', m.ReturnType.GetGenericArguments().[0])
    
            | MethodCall (_, MethodName "GroupBy" _, [expr'; Lambda ([paramExpr], bodyExpr) as f']) -> 
                GroupBy (optimize f' :?> LambdaExpression, (toQueryExpr << optimize) expr', typedefof<IGrouping<_, _>>.MakeGenericType [|paramExpr.Type; bodyExpr.Type|])
    
            | MethodCall (_, MethodName "OrderBy" _, [expr'; Lambda ([paramExpr], bodyExpr) as f']) -> 
                OrderBy (optimize f' :?> LambdaExpression, Order.Ascending, (toQueryExpr << optimize) expr', paramExpr.Type)
    
            | MethodCall (_, MethodName "Count" _,  [expr']) -> 
                let query' = (toQueryExpr << optimize) expr'
        ...
        and private transformer (expr : Expression) : Expression option =
            ...
            | MethodCall (_, MethodName "Select" _, [expr'; Lambda (param, bodyExpr) as f']) -> 
                let query = Transform (optimize f' :?> LambdaExpression, (toQueryExpr << optimize) expr', bodyExpr.Type)
                (Compiler.compileToSequential >> Some) query

            | MethodCall (_, MethodName "Select" _, [expr'; Lambda ([_; _], bodyExpr) as f']) -> 
                let query = TransformIndexed (optimize f' :?> LambdaExpression, (toQueryExpr << optimize) expr', bodyExpr.Type)
                (Compiler.compileToSequential >> Some) query

            | MethodCall (_, MethodName "Where" _, [expr'; Lambda ([paramExpr], _) as f']) -> 
                let query = Filter (optimize f' :?> LambdaExpression, (toQueryExpr << optimize) expr', paramExpr.Type)
                (Compiler.compileToSequential >> Some) query

            | MethodCall (_, MethodName "Where" _, [expr'; Lambda ([paramExpr; indexExpr], _) as f']) -> 
                let query = FilterIndexed (optimize f' :?> LambdaExpression, (toQueryExpr << optimize) expr', paramExpr.Type)
                (Compiler.compileToSequential >> Some) query
            ...
            | MethodCall (_, (MethodName "SelectMany" [|_; _|] as m), [expr'; Lambda ([paramExpr], bodyExpr)]) -> 
                let query = NestedQuery ((paramExpr, (toQueryExpr << optimize) bodyExpr), (toQueryExpr << optimize) expr', m.ReturnType.GetGenericArguments().[0])
                (Compiler.compileToSequential >> Some) query

            | MethodCall (_, MethodName "GroupBy" _, [expr'; Lambda ([paramExpr], bodyExpr) as f']) -> 
                let query = GroupBy (optimize f' :?> LambdaExpression, (toQueryExpr << optimize) expr', typedefof<IGrouping<_, _>>.MakeGenericType [|paramExpr.Type; bodyExpr.Type|])
                (Compiler.compileToSequential >> Some) query

            | MethodCall (_, MethodName "OrderBy" _, [expr'; Lambda ([paramExpr], bodyExpr) as f']) -> 
                let query = OrderBy (optimize f' :?> LambdaExpression, Order.Ascending, (toQueryExpr << optimize) expr', paramExpr.Type)
                (Compiler.compileToSequential >> Some) query
